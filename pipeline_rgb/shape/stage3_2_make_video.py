#!/usr/bin/env python3
"""
Create HD video from Stage-3.2 GP-shell visualization frames.
"""
...
IN_DIR = "pipeline_rgb/shape/per_frame_visualization_gp_shells"
OUT_FILE = f"{IN_DIR}/gp_shell_video.mp4"

def main():
    files = sorted([f for f in os.listdir(IN_DIR) if f.endswith(".ply")])

    if not files:
        print("No overlay PLYs found.")
        return
    ...
    for i, f in enumerate(files):
        ...
        fullpath = f"{IN_DIR}/{f}"
        cloud = add_cloud("cloud", fullpath)
