...
PRED_PATH = "data_local/results_Geometry/predictions_Geometry.ply"
CONF_PATH = "data_local/results_Geometry/confidence_Geometry.ply"
SAVE_DIR = "Evaluation/Geometry"
...
def load_ply_points_labels(path):
    ...
    # Label field detection (supports class / label / preds)
...
if __name__ == "__main__":
    print("\nStarting Evaluation: Geometry Scenario (single merged prediction)\n")
    ...
    if os.path.exists(CONF_PATH):
        ...
        # Scalar entropy proxy using only max-probability (same idea as RGB script)
        entropy_vals = -conf_values * np.log(conf_values + 1e-9)
        entropy_stats = {
            "min": float(np.min(entropy_vals)),
            "max": float(np.max(entropy_vals)),
    ...
    # ---------- Save Metrics ----------
    metrics_txt = os.path.join(SAVE_DIR, "metrics_Geometry.txt")
    with open(metrics_txt, "w") as f:
        f.write("=== Geometry Inference Evaluation (Single Prediction) ===\n\n")
    ...
    cm_plot = os.path.join(SAVE_DIR, "confusion_matrix_Geometry.png")
