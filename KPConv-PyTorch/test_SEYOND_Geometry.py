...
# --------------------- PATHS ---------------------
CHECKPOINT_PATH = (
    "training_logs_SEYOND_KPFCNN/Log_2025-11-10_11-31-41/"
    "checkpoints/best_chkp_adaptive.tar"
)
INPUT_DIR = "data_local/Input_Geometry"
OUTPUT_DIR = "data_local/results_Geometry"
...
class InputGeometryDataset(Dataset):
    """
    Minimal dataset for inference on data_local/Input_Geometry.
    ...
    def __getitem__(self, idx):
        ...
        Return one cloud in the format:
          points: (N,3)
          features: (N, in_features_dim) -> here all zeros, dim=5
          labels: (N,) dummy (zeros)
          cloud_index: int
        """
        f = self.files[idx]
        data = read_ply(f)
        ...
        # No RGB channels in geometry data, use all zeros but keep dim = 5
...
if __name__ == "__main__":
    ...
    # 2) Dataset + DataLoader on data_local/Input_Geometry
    print("\nData Preparation\n****************")
    ds = InputGeometryDataset(config, INPUT_DIR)
    ...
    # 3) Run inference
    print("\nStart SEYOND Test on Input_Geometry\n********************************")
    ...
    # 6) Save outputs in data_local/results_Geometry (no potentials)
    print("\nSaving results to data_local/results_Geometry/\n")

    # a) Raw predictions
    pred_ply = os.path.join(OUTPUT_DIR, "predictions_Geometry.ply")
    write_ply(
        pred_ply,
    ...
    color_ply = os.path.join(OUTPUT_DIR, "predictions_Geometry_colored.ply")
    write_ply(
        color_ply,
        [
            all_pts,
    ...
    # c) Confidence
    conf_ply = os.path.join(OUTPUT_DIR, "confidence_Geometry.ply")
    write_ply(
        conf_ply,
